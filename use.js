...
module.exports.run = async (bot, message, args) => {
  ...
  if (
    !message.channel.name.includes("指令") &&
    !message.channel.name.includes("賭場")
  )
    return message.reply("使用指令請至<#336341341053255684>。");
  ...
  if (!args[1]) {
    ...
    switch (args[1]) {
      case "energy":
        if (!items[id].items["003"]) return message.reply("沒有體力藥水。");
          ...
          return message.reply(
            "使用道具成功，道具剩餘數量:" + items[id].items["003"]
          );
        }
        break;
      ...
      case "license":
        if (!args[2]) {
          if (!items[id].items["000"] || items[id].items["000"] < 10)
            return message.reply("可供兌換的執照不足。");
          else {
            ...
            return message.reply(
              "兌換成功，執照剩餘數量:" + items[id].items["000"]
            );
          }
        }
        ...
        if (!Number.isInteger(num))
          return message.channel.send("請輸入正確整數。");
        if (num <= 10) return message.channel.send("請輸入正確整數。");
        if (!items[id].items["000"] || items[id].items["000"] < num)
          return message.reply("可供兌換的執照不足。");
        else {
