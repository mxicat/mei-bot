...
module.exports.run = async(bot, message, args) =>{
    ...
    if(!killed[id]) {killed[id] = { seal:0, time: 0 , ptime: 0 }}
  ...
  for(var i of Object.keys(killed)){ 
    if(killed[i].seal){
      if( time - killed[i].time > killed[i].ptime ){
          ...
          killed[i].time = 0;
          killed[i].ptime = 0;
          let muterole = message.guild.roles.find(x => x.name == "泡水海豹");
   ...
   if(!energy[id])  {energy[id] = {energy : 10};   fs.writeFileSync("./energy.json",JSON.stringify(energy));}
  
   if(!items[id])  
     items[id] = {items : {"000":0, "001":0, "002":0 , "003":0 , "004":0 , "005":0, "006":0}};   
  ...
  if(!weapons[id])  { weapons[id] = {weapons : {"B0":1}, status: {"now":"B0","lv":1, "exp":0,"re":0}};  fs.writeFileSync("./weapons.json",JSON.stringify(weapons));  }
  
  if(!bjlist[id]) {bjlist[id] = {op:0 , now:0, cards: new Array() , bet:0, re:0 , win:0 ,lose:0}};
  fs.writeFileSync("./bjlist.json",JSON.stringify(bjlist));  
  
  if(!betlist[id]) {betlist[id] = {now: 0,num:0}};
  fs.writeFileSync("./betlist.json",JSON.stringify(betlist));  
  
  if(!rolelist[id]) {rolelist[id] = {role: 0}};
  fs.writeFileSync("./rolelist.json",JSON.stringify(rolelist)); 
  
  if(!luckylist[id]) {luckylist[id] = { num: 0, reward: 0 , up: 1000 , down: 0}};
  fs.writeFileSync("./luckylist.json",JSON.stringify(luckylist)); 
  
  if(!points[id]) { points[id] = { str: 0, agi: 0, vit: 0, luk: 0, points: pp(id,man)}; }
  ...
  if(!abysslog[id]) { abysslog[id] = {now : 0 ,  level : 0}};
  ...
  if(!maridata[id]) {maridata[id] = {enable:0} };
